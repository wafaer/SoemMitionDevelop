...
typedef struct {
    ...
    int blending_next;      // segment is being blended into following segment 该片段正在与下一个片段进行混合
    ...
    int blend_prev;
    int accel_mode;
    int splitting;          // the segment is less than 1 cycle time away from the end.
